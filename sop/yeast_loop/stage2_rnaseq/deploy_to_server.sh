#!/usr/bin/env bash
# stage2_rnaseq SOP 落盘脚本（脱敏版，heredoc + sha256 自校验）
set -e
cd ~
mkdir -p ~/yeast_test/sop/yeast_loop/stage2_rnaseq
cd ~/yeast_test/sop/yeast_loop/stage2_rnaseq
cat > settings2.json <<'SKILL_EOF'
{
  "run_id": "batch01",
  "thread_budget": 32,
  ...
  "tools": {
    "hisat2": "${SHARED}/.conda/envs/hisat2/bin/hisat2",
    "hisat2_build": "${SHARED}/.conda/envs/hisat2/bin/hisat2-build",
    "samtools": "${SHARED}/.conda/envs/braker3/bin/samtools"
  },
  "hisat2_threads": 16,
  "sort_threads": 4,
  "hisat2_extra": ["--dta", "--no-unal"],
  "index_prefix": "SC288C",
  "samples": [
    {
      "id": "WT_Rep1",
      "srr": "SRR40431829",
      "r1": "${HOME}/yeast_test/0.Raw_Data/rnaseq/WT_Rep1_1.fq.gz",
      "r2": "${HOME}/yeast_test/0.Raw_Data/rnaseq/WT_Rep1_2.fq.gz"
    },
    {
      "id": "WT_Rep2",
      "srr": "SRR40431828",
      "r1": "${HOME}/yeast_test/0.Raw_Data/rnaseq/WT_Rep2_1.fq.gz",
      "r2": "${HOME}/yeast_test/0.Raw_Data/rnaseq/WT_Rep2_2.fq.gz"
    }
  ]
}
SKILL_EOF
cat > run_stage2.py <<'SKILL_EOF'
#!/usr/bin/env python3
...
44268cef9817e4a774f7f9989cc8fca2900d66fe2abdc9ec971fe28e77c69a56  settings2.json
d21feac4900ea3da63a9ae168622135572d9c93a285a68434707598bff32d216  run_stage2.py
17fdedb498b4ab7a8ef973a640b0426d504a610fc5d60289f7d8bd82f8c64bae  README.md
EXPECT_EOF
