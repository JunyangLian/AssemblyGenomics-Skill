...
### D-026 重新定位：从"新手 SOP 生成器"到"生信流程可靠性决策系统"
- 编号勘误：本条原记 D-024，与发布前审计条目（D-024）撞号，改为 D-026。
...
### D-027 三层验证体系（T3 规则验证 / T2 阶段级 / T1 端到端）+ 验证用例注册表
- 决策：按用户方案建立三层验证体系（docs/validation-roadmap.md）：**T3 规模化规则验证**（几十~上百公开物种，批量验证 QC 规则的特异性/敏感性，产出群体实测基线带）→ **T2 阶段级真实案例**（8-10 个，手头项目阶段产物回放，测继续/警告/回退判定，每用例必须预标注预期判定）→ **T1 核心端到端**（3-5 个差异维度精选：酵母✓、Siganus 鱼、葡萄三倍体、数据质量差案例）。配套 `docs/validation-cases/` 用例注册表（VC-*），首批注册酵母环 4 个判定点（全部 passed）。
- 理由：D-026 定位"可靠性决策系统"后，判定力需要三层独立验证——T3 验证规则输入、T2 验证判定输出、T1 验证端到端集成。T3 杠杆最高成本最低先做；T2 复用手头资产并行；T1 随算力。用例注册表让判定准确率第一次可度量。
- 影响：新增 docs/validation-roadmap.md + docs/validation-cases/；下一步工程 = 检查器批量调用改造（目录输入→判定表输出）+ NCBI 批量数据获取（D-017 网络纪律）；T2 需先做预标注（领域判断轮）。
