...
![工作流程总览](assets/workflow-overview.jpg)

从左到右：测序数据进来 → AI 教练识别数据、选定路线 → 生成带坑位预警（橙色警示标记）的 SOP 文档 → 你在自己的服务器上执行 → 回传成果校验（静默缺口检查 + 基线对照）→ 全部通过后沉淀为可复用的毕业包；校验不通过则返回修复脚本，逐段迭代直至交付。
